...
class EventRunner:
        ...
        try:
            ...
            while current_event_id:
                ...
                if not self.auto.running:
                    self.logger.info("检测到停止信号，退出事件循环")
                    return False
                
                # 检查是否暂停
                if self.auto.pause:
                    self.logger.info("检测到暂停信号，等待继续")
                    self.auto.pause_event.wait()  # 等待外部触发继续执行
                    continue
    ...
    def _execute_event(self, event: Event) -> Optional[str]:
        ...
        # 检查是否应该停止
        if not self.auto.running:
            self.logger.info("检测到停止信号，退出事件执行")
            return None
        ...
        if not event.conditions:
            ...
            self.logger.debug(f"执行事件: {event.name}, 动作完成，跳转到下一个事件: {event.next_event}")
    ...
    def _execute_actions(self, actions: List[Action]) -> Any:
        ...
        for action in actions:
            # 在每个动作执行前检查是否应该停止
            if not self.auto.running:
                self.logger.info("检测到停止信号，停止执行动作")
                return False
    ...
    def _execute_action(self, action: Action) -> Any:
        ...
        try:
            # 在执行任何动作前检查是否应该停止
            if not self.auto.running:
                self.logger.info("检测到停止信号，停止执行单个动作")
                return False
            ...
            if action.type == ActionType.SCREENSHOT:
                ...
                # 将等待时间分成多个小段，以便能够响应停止信号
                wait_step = 0.1  # 每次等待0.1秒
                wait_count = int(seconds / wait_step)
                remaining_time = seconds % wait_step
                
                # 先等待整数次的0.1秒
                for _ in range(wait_count):
                    if not self.auto.running:
                        self.logger.info("检测到停止信号，中断等待")
                        return False
